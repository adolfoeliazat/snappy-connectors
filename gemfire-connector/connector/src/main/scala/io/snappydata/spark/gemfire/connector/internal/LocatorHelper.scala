...
import com.gemstone.gemfire.cache.client.internal.locator.{GetAllServersRequest, GetAllServersResponse}
...
object LocatorHelper extends Logging {
  ...
  def getAllGeodeServers(locators: Seq[(String, Int)], serverGroup: String = ""): Option[Seq[(String, Int)]] = {
    ...
    locators.find { case (host, port) =>
      try {
        ...
        case e: Exception => logWarning("getAllGeodeServers error", e)
      }
      result.isDefined
    }
    result
  ...
  def pickPreferredGeodeServers(
    ...
    if (firstIdx < 0) {
      ...
      val seq = if (firstIdx == lastIdx) {
        // one local server
        seq2 ++ seq1
      } else {
        // multiple local server
