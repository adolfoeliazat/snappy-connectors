...
import io.snappydata.spark.gemfire.connector.{GemFireConnection, NumberPartitionsPerServerPropKey}
...
object ServerSplitsPartitioner extends GemFireRDDPartitioner {
  ...
  : Array[Partition] = {
    ...
    val srvToFinalBucketSet = if (emptyIDs.isEmpty) srvToSplitedBuckeSet
      ...
      emptyIDs.grouped(groupSize(emptyIDs.size, srvToSplitedBuckeSet.size)).toList, (Nil, Set.empty), Set.empty).map { case ((server, set1), set2) => (server, SortedSet[Int]() ++ set1 ++ set2) }

    // 5. create array of partitions w/ 0-based index
    (0 until srvToFinalBucketSet.size).toList.zip(srvToFinalBucketSet).map { case (i, (srv, set)) => {
