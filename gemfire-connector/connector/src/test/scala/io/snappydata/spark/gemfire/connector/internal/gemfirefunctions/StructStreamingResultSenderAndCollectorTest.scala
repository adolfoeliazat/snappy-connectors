...
import com.gemstone.gemfire.internal.{ByteArrayDataInput, HeapDataOutputStream}
...
class StructStreamingResultSenderAndCollectorTest extends FunSuite with BeforeAndAfter {
  ...
  test("transfer data with 10K rows with 2 sender") {
    ...
    val total = 300
    val sender1 = Future {
      new StructStreamingResultSender(baseSender, OneColType, intIterator(1, total / 2, genExcp = false), "sender1").send()
    }
    val sender2 = Future {
      new StructStreamingResultSender(baseSender, OneColType, intIterator(total / 2 + 1, total / 2, genExcp = false), "sender2").send()
    }
  ...
  test("transfer data with 10K rows with 2 sender with error") {
    ...
    val sender2 = Future {
      new StructStreamingResultSender(baseSender, OneColType, intIterator(total / 2 + 1, total / 2, genExcp = true), "sender2").send()
    }
    Await.result(sender1, 1 seconds)
    Await.result(sender2, 1 seconds)
  ...
  test("DataSerializer usage") {
    ...
    inBuf.initialize(bytesHello, Version.CURRENT_GFE)
    ...
    inBuf.initialize(outBuf.toByteArray.clone(), Version.CURRENT_GFE)
    val bytesHello2: Array[Byte] = DataSerializer.readByteArray(inBuf)
    inBuf.initialize(bytesHello2, Version.CURRENT_GFE)
