 ...
 *              }
 */
public class NonVersionedHeapDataOutputStream extends OutputStream implements
  ...
  public NonVersionedHeapDataOutputStream() {
   ...
   */
  public NonVersionedHeapDataOutputStream(String s) {
    int maxStrBytes;
    if (ASCII_STRINGS) {
  ...
  public NonVersionedHeapDataOutputStream(int allocSize) {
    if (allocSize < 32) {
   ...
   * @param bytes
   */
  public NonVersionedHeapDataOutputStream(byte[] bytes) {
   ...
   */
  public NonVersionedHeapDataOutputStream(byte[] bytes, int offset, int length) {
    if (length <= 0) {
      throw new IllegalArgumentException("The byte array must not be empty");
    }
    if (length > 32) {
  ...
  private final class HDInputStream extends InputStream {
    ...
    public HDInputStream() {
      finishWriting();
      if (NonVersionedHeapDataOutputStream.this.chunks != null) {
        this.chunkIt = NonVersionedHeapDataOutputStream.this.chunks.iterator();
        nextChunk();
      } else {
        this.chunkIt = null;
        this.bb = NonVersionedHeapDataOutputStream.this.buffer;
      }
    }
    ...
    private void consume(int c) {
      NonVersionedHeapDataOutputStream.this.size -= c;
    }
...
//       this.buffer.put(bytes);
  ...
  public void writeAsSerializedByteArray(Object v) throws IOException {
    ...
    checkIfWritable();
    ensureCapacity(5);
    if (v instanceof NonVersionedHeapDataOutputStream) {
      NonVersionedHeapDataOutputStream other = (NonVersionedHeapDataOutputStream)v;
