...
class BasicIntegrationTest extends FunSuite with Matchers with BeforeAndAfterAll with GemFireCluster {
  ...
  override def beforeAll() {
    ...
    val conf = new SparkConf()
        ...
        .set(DistributionConfig.REMOTE_LOCATORS_NAME, s"localhost[$locatorPort]")
        .set("spark.serializer", "org.apache.spark.serializer.KryoSerializer")
        .set("spark.kryo.registrator", "io.snappydata.spark.gemfire.connector.GemFireKryoRegistrator")
  ...
  private def simpleQuery(regionName: String) {
    ...
    val sqlRS: Array[Any] = SQLResult.map(r => Row(r(0)))(RowEncoder(StructType(Seq(SQLResult.schema
        .fields(0))))).collect().map(_ (0))
    ...
    //Verify the SQL query result, r(0) mean column 0
    val sqlRS2: Array[Any] = SQLResult2.map(r => Row(r(0)))(RowEncoder(StructType(Seq(SQLResult2
        .schema
        .fields(0))))).collect().map(_ (0))
  ...
  private def queryUDT(regionName: String) {
    ...
    val sqlRS = SQLResult.map(r => Row(r(0)))(RowEncoder(StructType(Seq(SQLResult.schema
        .fields(0))))).collect().map(_ (0))
  ...
  private def getQueryRDD[T: ClassTag](query: String): QueryRDD[T] =
    new QueryRDD[T](sc, query)
  ...
  private def complexUDTDataFrame(regionName: String) {
    ...
    //Create DataFrame using GemFire OQL
    val sqlContext = new org.apache.spark.sql.SQLContext(sc)
    val dataFrame = sqlContext.gemfireOQL(s"SELECT DISTINCT * FROM /$regionName WHERE status = " +
        s"'active'")
    dataFrame.registerTempTable("Portfolio")
  ...
  private def queryComplexUDTProjectionDataFrame(regionName: String) {
    ...
    val dataFrame = sqlContext.gemfireOQL(
      s"""SELECT id, "type", positions, status FROM
          |/$regionName WHERE status = 'active'""".stripMargin)
    dataFrame.registerTempTable("Portfolio")
  ...
  private def queryComplexUDTNestProjectionDataFrame(regionName: String) {
    ...
    val sqlContext = new org.apache.spark.sql.SQLContext(sc)
    val dataFrame = sqlContext.gemfireOQL(
      s"""SELECT r.id, r."type", r.positions, r.status FROM
          |/$regionName r, r.positions.values f WHERE r.status = 'active' and f.secId = 'MSFT'""".stripMargin)
    dataFrame.registerTempTable("Portfolio")
  ...
  test("RDD.saveToGemFire") {
    ...
    // verify
    val region: Region[String, String] = DefaultGemFireConnectionManager.getConnection.getRegionProxy(regionName)
